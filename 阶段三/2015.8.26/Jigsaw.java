	...
	public boolean BFSearch() throws IOException {
        ...
        while(!openList.isEmpty()) {
        	if (openList.firstElement().equals(endJNode)) {
        		...
        		Vector<JigsawNode> findAjacent = findFollowJNodes(currentJNode);
                for (JigsawNode node : findAjacent) {
	...
	private void estimateValue(JigsawNode jNode) {
		...
		for(int i = 0; i <= dimension*dimension; i++) {
			...
			if (jNode.getNodesState()[i]!=0) {
				int currentRow = (i-1)/dimension;
				int currentCol = (i-1)%dimension;
				int goalRow = (jNode.getNodesState()[i]-1)/dimension;
				int goalCol = (jNode.getNodesState()[i]-1)%dimension;
				xdistance = Math.abs(currentCol-goalCol);
				ydistance = Math.abs(currentRow-goalRow);
				//distance1 = Math.sqrt(xdistance*xdistance + ydistance*ydistance);
				//sumOfDistance += distance1;
				distance2 += (xdistance+ydistance);
			}
		}
		
		jNode.setEstimatedValue(s+distance2);
	}
